...
def test_bad_capability(mock_deploy):
    ...
    with pytest.raises(Exception):
        runner.invoke(cli.cfhelper, ['--config', os.path.join(CONFIG_DIR, 'valid_singlestack_bad_capability.cfh'), 'deploy', 'MyStackAlias'], catch_exceptions=False)
...
def test_with_capability(mock_deploy):
    ...
    runner.invoke(cli.cfhelper, ['--config', os.path.join(CONFIG_DIR, 'valid_singlestack_with_capability.cfh'), 'deploy', 'MyStackAlias'], catch_exceptions=False)
    mock_deploy.assert_called_once_with('MyStackName', mock.ANY, False, {'CAPABILITY_IAM'})
...
def test_with_multiple_capabilities(mock_deploy):
    ...
    runner.invoke(cli.cfhelper, ['--config', os.path.join(CONFIG_DIR, 'valid_singlestack_with_multiple_capabilities.cfh'), 'deploy', 'MyStackAlias'], catch_exceptions=False)
    mock_deploy.assert_called_once_with('MyStackName', mock.ANY, False, {'CAPABILITY_IAM', 'CAPABILITY_NAMED_IAM'})
