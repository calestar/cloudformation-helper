...
def test_stack_wrong_name(mock_aws_stack_exists, mock_boto3):
    ...
    with pytest.raises(Exception, match=r"Could not find stack config named 'IamNotAStack'"):
        runner.invoke(cli.cfhelper, ['--config', os.path.join(CONFIG_DIR, 'valid_multistacks.cfh'), 'deploy', 'IamNotAStack'], catch_exceptions=False)


@mock.patch('cloudformation_helper.utils.aws.boto3')
@mock.patch.object(aws, 'stack_exists', return_value=True)
@mock.patch.object(aws, 'has_changeset', return_value=True)
def test_update_with_existing_changeset(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
...
@mock.patch.object(aws, 'stack_exists', return_value=True)
@mock.patch.object(aws, 'has_changeset', return_value=True)
def test_update_with_existing_changeset_and_continue(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
