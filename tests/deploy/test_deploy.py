...
def test_create_with_capabilities(mock_aws_stack_exists, mock_boto3):
    ...
    runner.invoke(
        cli.cfhelper,
        ['--config', os.path.join(CONFIG_DIR, 'valid_singlestack_with_multiple_capabilities.cfh'), 'deploy', 'MyStackAlias'],
        catch_exceptions=False
    )
...
@mock.patch.object(aws, 'stack_exists', return_value=False)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_create_with_changeset_and_capabilities(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
    """Create a new stack using changesets"""
    runner = CliRunner()

    runner.invoke(
        cli.cfhelper,
        ['--config', os.path.join(CONFIG_DIR, 'valid_singlestack_capabilities_and_changeset.cfh'), 'deploy', 'MyStackAlias'],
        catch_exceptions=False,
        input='y\n',
    )
