...
def test_update(mock_aws_stack_exists, mock_boto3):
    ...
    runner.invoke(cli.cfhelper, ['--config', os.path.join(CONFIG_DIR, 'valid_multistacks.cfh'), 'deploy', 'MyStackAlias'], catch_exceptions=False)
    mock_boto3.client.return_value.update_stack.assert_called_once_with(StackName='MyStackName', TemplateBody=mock.ANY, Capabilities=mock.ANY)


@mock.patch('cloudformation_helper.utils.aws.boto3')
@mock.patch.object(aws, 'stack_exists', return_value=False)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_create_with_changeset(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
    """Create a new stack using changesets"""
    runner = CliRunner()

    runner.invoke(
        cli.cfhelper,
        ['--config', os.path.join(CONFIG_DIR, 'valid_with_changeset.cfh'), 'deploy', 'MyStackAlias'],
    ...
    mock_boto3.client.return_value.execute_change_set.assert_called_once_with(StackName='MyStackName', ChangeSetName=aws.CHANGESET_NAME)


@mock.patch('cloudformation_helper.utils.aws.boto3')
@mock.patch.object(aws, 'stack_exists', return_value=False)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_create_with_changeset_no_execute(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
...
@mock.patch.object(aws, 'stack_exists', return_value=False)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_create_with_changeset_no_execute_cleanup(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
    ...
    mock_boto3.client.return_value.execute_change_set.assert_not_called()
    mock_boto3.client.return_value.delete_change_set.assert_called_once_with(StackName='MyStackName', ChangeSetName=aws.CHANGESET_NAME)


@mock.patch('cloudformation_helper.utils.aws.boto3')
@mock.patch.object(aws, 'stack_exists', return_value=True)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_update_with_changeset(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
...
@mock.patch.object(aws, 'stack_exists', return_value=True)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_update_with_changeset_no_execute(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
...
@mock.patch.object(aws, 'stack_exists', return_value=True)
@mock.patch.object(aws, 'has_changeset', return_value=False)
def test_update_with_changeset_no_execute_cleanup(mock_aws_has_changeset, mock_aws_stack_exists, mock_boto3):
